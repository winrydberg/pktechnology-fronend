...
export default {
  ...
  methods:{
    ...
    getAllLatestWorkOrders(){
      apiClient.get<WorkOrderResponse>('/api/workorder/getworkorder').then((response) => {
        this.loading = false;
        if(response.status === 200 && response.data.code === "200") {
          this.latestWorkOrders = response.data.data.results;
        }else{
